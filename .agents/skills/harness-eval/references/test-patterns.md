...
async def test_hook_blocks():
    ...
    from nexus.hooks.loader import HookRegistry
    from nexus.hooks.schemas import CommandHookDefinition
    from nexus.hooks.executor import HookExecutor, HookExecutionContext
...
async def test_concurrent_teammates():
    from nexus.swarm.in_process import start_in_process_teammate, TeammateAbortController
    from nexus.swarm.types import TeammateSpawnConfig
    from nexus.engine.query import QueryContext
...
async def test_session_resume():
    from nexus.services.session_storage import save_session_snapshot, load_session_snapshot
