<h1 align="center"><img src="assets/logo.png" alt="Nexus" width="64" style="vertical-align: middle;">&nbsp; <code>oh</code> — Nexus: Open Agent Harness</h1>
...
**Nexus** delivers core lightweight agent infrastructure: tool-use, skills, memory, and multi-agent coordination.
...
<p align="center">
  ...
  <a href="https://github.com/HKUDS/Nexus/actions/workflows/ci.yml"><img src="https://github.com/HKUDS/Nexus/actions/workflows/ci.yml/badge.svg" alt="CI"></a>
  <a href="https://github.com/HKUDS/.github/blob/main/profile/README.md"><img src="https://img.shields.io/badge/Feishu-Group-E9DBFC?style=flat&logo=feishu&logoColor=white" alt="Feishu"></a>
  <a href="https://github.com/HKUDS/.github/blob/main/profile/README.md"><img src="https://img.shields.io/badge/WeChat-Group-C5EAB4?style=flat&logo=wechat&logoColor=white" alt="WeChat"></a>
</p>

One Command (**oh**) to Launch **Nexus** and Unlock All Agent Harnesses. 

Supports CLI agent integration including OpenClaw, nanobot, Cursor, and more.

<p align="center">
  <img src="assets/cli-typing.gif" alt="Nexus Terminal Demo" width="800">
</p>
...
Nexus is an open-source Python implementation designed for **researchers, builders, and the community**:
...
- **2026-04-06** 🚀 **v0.1.2** — Unified setup flows and `ohmo` personal-agent app:
  - `nx setup` now guides provider selection as workflows instead of exposing raw auth/provider internals
...
- **2026-04-01** 🎨 **v0.1.0** — Initial **Nexus** open-source release featuring complete Harness architecture: 
...
curl -fsSL https://raw.githubusercontent.com/HKUDS/Nexus/main/scripts/install.sh | bash -s -- --from-source

# Install with IM channel support
curl -fsSL https://raw.githubusercontent.com/HKUDS/Nexus/main/scripts/install.sh | bash -s -- --with-channels
...
3. Install Nexus via `pip`
4. Set up the React TUI (`npm install`) if Node.js is available
5. Create `~/.nexus/` config directory
6. Confirm with `nx --version`
...
```bash
ANTHROPIC_API_KEY=your_key uv run nx -p "Inspect this repository and list the top 3 refactors"
```
...
# Launch
nx                    # if venv is activated
uv run nx             # without activating venv
```
...
Nexus stores API-key-backed compatible profiles with profile-scoped credentials when appropriate, so different compatible endpoints do not have to share one global key.

<p align="center">
  <img src="assets/landing.png" alt="Nexus Landing Screen" width="700">
</p>
...
Nexus treats providers as **workflows** backed by named profiles. In day-to-day use, prefer:
...
For custom compatible endpoints, Nexus can bind credentials per profile instead of forcing every Anthropic-compatible or OpenAI-compatible backend to share the same API key.
...
nx auth copilot-login

# Then launch with Copilot as the provider
uv run nx --api-format copilot
...
Nexus implements the core Agent Harness pattern with 10 subsystems:
...
**Compatible with [anthropics/skills](https://github.com/anthropics/skills)** — just copy `.md` files to `~/.nexus/skills/`.
...
# Manage plugins
nx plugin list
nx plugin install <source>
nx plugin enable <name>
```

### 🤝 Ecosystem Workflows

Nexus is useful as a lightweight harness layer around Claude-style tooling conventions:

- **OpenClaw-oriented workflows** can reuse Markdown-first knowledge and command-driven collaboration patterns.
- **Claude-style plugins and skills** stay portable because Nexus keeps those formats familiar.
...
Subcommands: nx setup | nx provider | nx auth | nx mcp | nx plugin
```

### 🧑‍💼 ohmo Personal Agent

`ohmo` is a personal-agent app built on top of Nexus. It is packaged alongside `oh`, with its own workspace and gateway:
...
`ohmo config` uses the same workflow language as `nx setup`, so you can point the personal-agent gateway at:
...
from nexus.tools.base import BaseTool, ToolExecutionContext, ToolResult
...
Create `~/.nexus/skills/my-skill.md`:
...
Create `.nexus/plugins/my-plugin/.claude-plugin/plugin.json`:
...
Nexus is most useful when treated as a small, inspectable harness you can adapt to a real workflow:
...
Nexus is a **community-driven research project**. We welcome contributions in:
...
<p align="center">
  <img src="assets/logo.png" alt="Nexus" width="48">
  <br>
...
<div align="center">
  <a href="https://star-history.com/#HKUDS/Nexus&Date">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=HKUDS/Nexus&type=Date&theme=dark" />
      <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=HKUDS/Nexus&type=Date" />
      <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=HKUDS/Nexus&type=Date" style="border-radius: 15px; box-shadow: 0 0 30px rgba(0, 217, 255, 0.3);" />
    </picture>
  </a>
</div>

<p align="center">
  <em> Thanks for visiting ✨ Nexus!</em><br><br>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=HKUDS.Nexus&style=for-the-badge&color=00d4ff" alt="Views">
</p>
