"""CLI entry point for the nexus personal-agent app."""
...
from personal_agent.auth.manager import AuthManager
from personal_agent.config import load_settings

from personal_agent.gateway.config import load_gateway_config, save_gateway_config
from personal_agent.gateway.models import GatewayConfig
from personal_agent.gateway.service import (
    NexusGatewayService,
    gateway_status,
    start_gateway_process,
    stop_gateway_process,
)
from personal_agent.memory import add_memory_entry, list_memory_files, remove_memory_entry
from personal_agent.runtime import launch_nexus_react_tui, run_nexus_backend, run_nexus_print_mode
from personal_agent.session_storage import NexusSessionBackend
...
app = typer.Typer(
    name="nexus",
    help="nexus: a personal-agent app built on top of Nexus.",
    invoke_without_command=True,
    add_completion=False,
)
memory_app = typer.Typer(name="memory", help="Manage .nexus memory")
soul_app = typer.Typer(name="soul", help="Inspect or edit soul.md")
user_app = typer.Typer(name="user", help="Inspect or edit user.md")
gateway_app = typer.Typer(name="gateway", help="Run the nexus gateway")
...
_WORKSPACE_HELP = "Path to the nexus workspace (defaults to ~/.nexus)"
...
def _prompt_provider_profile(workspace: str | Path) -> str:
    ...
    if _can_use_questionary():
        ...
        result = questionary.select("Choose provider profile for nexus:", choices=choices, default=default_value).ask()
        if result is None:
            raise typer.Abort()
        return str(result)
    ...
    return _select_from_menu(
        "Choose provider profile for nexus:",
        options,
...
def _prompt_channels(existing: GatewayConfig) -> tuple[list[str], dict[str, dict]]:
    ...
    print("Configure channels for nexus gateway:")
...
def _maybe_restart_gateway(*, cwd: str | Path, workspace: str | Path) -> None:
    ...
    if not _confirm_prompt("Gateway is running. Restart now to apply changes?", default=True):
        print("Configuration saved. Restart later with `nexus gateway restart`.")
        return
    stop_gateway_process(cwd, workspace)
    pid = start_gateway_process(cwd, workspace)
    print(f"nexus gateway restarted (pid={pid})")
...
def main(
    ...
    resume: str | None = typer.Option(None, "--resume", help="Resume an nexus session by id"),
    continue_session: bool = typer.Option(False, "--continue", help="Continue the latest nexus session"),
) -> None:
    """Launch the nexus app or invoke a subcommand."""
    if ctx.invoked_subcommand is not None:
        return

    cwd_path = str(Path(cwd).resolve())
    workspace_root = initialize_workspace(workspace)
    backend = NexusSessionBackend(workspace_root)
    restore_messages = None
    if continue_session:
        latest = backend.load_latest(cwd_path)
        if latest is None:
            print("No previous nexus session found in this directory.", file=sys.stderr)
            raise typer.Exit(1)
        restore_messages = latest.get("messages")
    elif resume:
        snapshot = backend.load_by_id(cwd_path, resume)
        if snapshot is None:
            print(f"nexus session not found: {resume}", file=sys.stderr)
            raise typer.Exit(1)
...
def init_cmd(
    ...
    """Initialize the .nexus workspace."""
    root_path = get_workspace_root(workspace)
    already_exists = root_path.exists()
    root = initialize_workspace(root_path)
    print(f"Initialized nexus workspace at {root}")
    if already_exists:
        print("nexus workspace already exists.")
        if not interactive:
            print("Use `nexus config` to update provider and channel settings.")
            return
        if not _confirm_prompt("Open configuration now?", default=True):
            print("Use `nexus config` when you want to change provider or channel settings.")
            return
    if interactive:
...
def doctor_cmd(
    ...
    """Check .nexus workspace and provider readiness."""
    cwd_path = str(Path(cwd).resolve())
    workspace_root = initialize_workspace(workspace)
...
def gateway_run_cmd(
    ...
    """Run the nexus gateway in the foreground."""
    _configure_gateway_logging(workspace)
    service = NexusGatewayService(cwd, workspace)
...
def gateway_start_cmd(
    ...
    pid = start_gateway_process(cwd, workspace)
    print(f"nexus gateway started (pid={pid})")
...
def gateway_stop_cmd(
    ...
    if stop_gateway_process(cwd, workspace):
        print("nexus gateway stopped.")
        return
    print("nexus gateway is not running.")
