"""Gateway bridge connecting channel bus traffic to nexus runtimes."""
...
from nexus.channels.bus.events import OutboundMessage
from nexus.channels.bus.queue import MessageBus

from personal_agent.gateway.router import session_key_for_message
from personal_agent.gateway.runtime import NexusSessionRuntimePool
...
def _format_gateway_error(exc: Exception) -> str:
    ...
    if "claude oauth refresh failed" in lowered:
        return (
            "[nexus gateway error] Claude subscription auth refresh failed. "
            "Run `nx auth claude-login` again or switch the gateway profile."
        )
    if "claude oauth refresh token is invalid or expired" in lowered:
        return (
            "[nexus gateway error] Claude subscription token is expired. "
            "Run `claude auth login`, then `nx auth claude-login`, or switch the gateway profile."
        )
    if "auth source not found" in lowered or "access token" in lowered:
        return (
            "[nexus gateway error] Authentication is not configured for the current "
            "gateway profile. Run `nx setup` or `nexus config`."
        )
    if "api key" in lowered or "auth" in lowered or "credential" in lowered:
        return (
            "[nexus gateway error] Authentication failed for the current gateway "
            "profile. Check `nx auth status` and `nexus config`."
        )
    return f"[nexus gateway error] {message}"


class NexusGatewayBridge:
    ...
    def __init__(
        ...
        *,
        bus: MessageBus,
        runtime_pool: NexusSessionRuntimePool,
    ...
    async def run(self) -> None:
        ...
        while self._running:
            ...
            logger.info(
                "nexus inbound received channel=%s chat_id=%s sender_id=%s session_key=%s content=%r",
                message.channel,
                message.chat_id,
                message.sender_id,
                session_key,
            ...
            task = asyncio.create_task(
                ...
                name=f"nexus-session:{session_key}",
            )
    ...
    async def _process_message(self, message, session_key: str) -> None:
        try:
            ...
            async for update in self._runtime_pool.stream_message(message, session_key):
                ...
                if not update.text:
                    continue
                logger.info(
                    "nexus outbound update channel=%s chat_id=%s session_key=%s kind=%s content=%r",
                    message.channel,
                    message.chat_id,
                    session_key,
                    update.kind,
            ...
            logger.info(
                "nexus session interrupted channel=%s chat_id=%s session_key=%s reason=%s",
                message.channel,
                message.chat_id,
                session_key,
            ...
            logger.exception(
                "nexus gateway failed to process inbound message channel=%s chat_id=%s sender_id=%s session_key=%s content=%r",
                message.channel,
                message.chat_id,
                message.sender_id,
                session_key,
        ...
        if not reply:
            logger.info(
                "nexus inbound finished without final reply channel=%s chat_id=%s session_key=%s",
                message.channel,
                message.chat_id,
                session_key,
            )
            return
        logger.info(
            "nexus outbound final channel=%s chat_id=%s session_key=%s content=%r",
            message.channel,
            message.chat_id,
            session_key,
