"""Session-aware runtime pool for nexus gateway."""
...
from nexus.channels.bus.events import InboundMessage
from nexus.commands import CommandContext
from nexus.engine.messages import ConversationMessage, ImageBlock, TextBlock
from nexus.engine.query import MaxTurnsExceeded
...
from nexus.prompts import build_runtime_system_prompt
from nexus.ui.runtime import RuntimeBundle, _last_user_text, build_runtime, close_runtime, start_runtime

from personal_agent.prompts import build_nexus_system_prompt
from personal_agent.session_storage import NexusSessionBackend
from personal_agent.workspace import get_plugins_dir, get_skills_dir, initialize_workspace
...
class NexusSessionRuntimePool:
    ...
    def __init__(
        ...
        *,
        ...
        self._session_backend = NexusSessionBackend(self._workspace)
    ...
    async def get_bundle(self, session_key: str, latest_user_prompt: str | None = None) -> RuntimeBundle:
        ...
        if bundle is not None:
            logger.info(
                "nexus runtime reusing session session_key=%s session_id=%s prompt=%r",
                session_key,
                bundle.session_id,
        ...
        logger.info(
            "nexus runtime creating session session_key=%s restored=%s prompt=%r",
            session_key,
            bool(snapshot),
            _content_snippet(latest_user_prompt or ""),
        )
        bundle = await build_runtime(
            ...
            system_prompt=build_nexus_system_prompt(self._cwd, workspace=self._workspace, extra_prompt=None),
        ...
        logger.info(
            "nexus runtime started session_key=%s session_id=%s restored_messages=%s",
            session_key,
            bundle.session_id,
    ...
    async def stream_message(self, message: InboundMessage, session_key: str):
        ...
        logger.info(
            "nexus runtime processing start channel=%s chat_id=%s session_key=%s session_id=%s content=%r",
            message.channel,
            message.chat_id,
            session_key,
            bundle.session_id,
    ...
    async def _stream_engine_message(
        ...
        *,
        ...
        if reply:
            logger.info(
                "nexus runtime processing complete session_key=%s session_id=%s reply=%r",
                session_key,
                bundle.session_id,
    ...
    async def _convert_stream_event(
        ...
        *,
        ...
        if isinstance(event, CompactProgressEvent):
            logger.info(
                "nexus runtime compact progress session_key=%s session_id=%s phase=%s trigger=%s attempt=%s",
                session_key,
                bundle.session_id,
                event.phase,
                event.trigger,
        ...
        if isinstance(event, StatusEvent):
            logger.info(
                "nexus runtime status session_key=%s session_id=%s message=%r",
                session_key,
                bundle.session_id,
        ...
        if isinstance(event, ToolExecutionStarted):
            ...
            logger.info(
                "nexus runtime tool start session_key=%s session_id=%s tool=%s summary=%r",
                session_key,
                bundle.session_id,
                event.tool_name,
                summary,
        ...
        if isinstance(event, ToolExecutionCompleted):
            logger.info(
                "nexus runtime tool complete session_key=%s session_id=%s tool=%s",
                session_key,
                bundle.session_id,
                event.tool_name,
            )
        ...
        if isinstance(event, ErrorEvent):
            logger.error(
                "nexus runtime error session_key=%s session_id=%s message=%r",
                session_key,
                bundle.session_id,
                _content_snippet(event.message),
            )
    ...
    async def _save_snapshot(self, bundle: RuntimeBundle, session_key: str, user_prompt: str) -> None:
        ...
        logger.info(
            "nexus runtime saved snapshot session_key=%s session_id=%s message_count=%s",
            session_key,
            bundle.session_id,
    ...
    async def _refresh_bundle(
        ...
        logger.info(
            "nexus runtime refreshed session_key=%s session_id=%s message_count=%s",
            session_key,
    ...
    def _runtime_system_prompt(self, bundle: RuntimeBundle, latest_user_prompt: str | None) -> str:
        ...
        if not hasattr(settings, "system_prompt"):
            return build_nexus_system_prompt(self._cwd, workspace=self._workspace, extra_prompt=None)
...
def _build_inbound_user_message(message: InboundMessage) -> ConversationMessage:
    ...
    for media_path in message.media:
        ...
        try:
            ...
            logger.exception("nexus runtime failed to encode image attachment path=%s", media_path)
