"""Gateway service lifecycle for nexus."""
...
from personal_agent.gateway.bridge import NexusGatewayBridge
from personal_agent.gateway.config import build_channel_manager_config, load_gateway_config
from personal_agent.gateway.models import GatewayState
...
class NexusGatewayService:
    ...
    def __init__(self, cwd: str | Path | None = None, workspace: str | Path | None = None) -> None:
        ...
        self._runtime_pool = NexusSessionRuntimePool(
            cwd=self._cwd,
        ...
        self._bridge = NexusGatewayBridge(
            bus=self._bus,
    ...
    def _exec_restart(self) -> None:
        ...
        logger.info("nexus gateway restarting in-place argv=%s", argv)
    ...
    async def _publish_pending_restart_notice(self) -> None:
        ...
        try:
            ...
            logger.info(
                "nexus gateway published restart confirmation channel=%s chat_id=%s session_key=%s",
                channel,
                chat_id,
                session_key,
            )
    ...
    async def run_foreground(self) -> int:
        ...
        bridge_task = asyncio.create_task(self._bridge.run(), name="nexus-gateway-bridge")
        manager_task = asyncio.create_task(self._manager.start_all(), name="nexus-gateway-channels")
        restart_notice_task = asyncio.create_task(
            self._publish_pending_restart_notice(),
            name="nexus-gateway-restart-notice",
        )
...
def _iter_workspace_gateway_pids(workspace: str | Path | None = None) -> list[int]:
    ...
    for line in result.stdout.splitlines():
        ...
        if pid == current_pid:
            continue
        if "-m nexus gateway run" not in args:
            continue
