"""Workspace helpers for the nexus personal-agent app."""
...
WORKSPACE_DIRNAME = ".personal_agent"

SOUL_TEMPLATE = """# SOUL.md - Who You Are

You are Nexus, a personal agent built on top of Nexus.
...
How should Nexus show up for this user over time?
...
clearer shared sense of who Nexus is.
"""
...
def get_workspace_root(workspace: str | Path | None = None) -> Path:
    """Return the nexus workspace root.

    Resolution order:
    1. Explicit ``workspace`` argument
    2. ``NEXUS_WORKSPACE`` environment variable
    3. ``~/.nexus``
    """
    explicit = workspace or os.environ.get("NEXUS_WORKSPACE")
    if explicit:
...
def initialize_workspace(workspace: str | Path | None = None) -> Path:
    ...
    state_data = {"app": "nexus", "workspace": str(root.resolve())}
