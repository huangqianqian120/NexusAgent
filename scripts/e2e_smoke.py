#!/usr/bin/env python3
"""Run real end-to-end Nexus scenarios against an Anthropic-compatible API."""
...
from nexus.mcp.types import McpStdioServerConfig
from nexus.memory import add_memory_entry
...
def _setup_plugin_combo_flow(cwd: Path, _: Path) -> None:
    plugin_dir = cwd / ".nexus" / "plugins" / "fixture-plugin"
...
def _validate_worktree_flow(
    ...
    worktree_path = cwd / ".nexus" / "worktrees" / "smoke-worktree"
...
SCENARIOS: dict[str, Scenario] = {
    "file_io": Scenario(
        name="file_io",
        prompt=(
            "You are running an Nexus smoke test. "
            "You must use tools. "
    ...
    "search_edit": Scenario(
        name="search_edit",
        prompt=(
            "You are running an Nexus search and edit test. "
            "You must use tools. "
    ...
    "phase48": Scenario(
        name="phase48",
        prompt=(
            "You are running an Nexus Phase 4-8 smoke test. "
            "You must use tools. "
    ...
    "task_flow": Scenario(
        name="task_flow",
        prompt=(
            "You are running an Nexus background task test. "
            "You must use tools. "
    ...
    "skill_flow": Scenario(
        name="skill_flow",
        prompt=(
            "You are running an Nexus skill loading test. "
            "You must use tools. "
    ...
    "mcp_model": Scenario(
        name="mcp_model",
        prompt=(
            "You are running an Nexus MCP integration test. "
            "You must use tools. "
    ...
    "mcp_resource": Scenario(
        name="mcp_resource",
        prompt=(
            "You are running an Nexus MCP resource test. "
            "You must use tools. "
    ...
    "agent_flow": Scenario(
        name="agent_flow",
        prompt=(
            "You are running an Nexus agent delegation test. "
            "You must use tools. "
    ...
    "remote_agent_flow": Scenario(
        ...
        prompt=(
            "You are running an Nexus remote-agent mode test. "
            "You must use tools. "
    ...
    "plugin_combo": Scenario(
        name="plugin_combo",
        prompt=(
            "You are running an Nexus plugin combo test. "
            "You must use tools. "
    ...
    "ask_user_flow": Scenario(
        name="ask_user_flow",
        prompt=(
            "You are running an Nexus ask-user test. "
            "You must use tools. "
    ...
    "task_update_flow": Scenario(
        ...
        prompt=(
            "You are running an Nexus task update test. "
            "You must use tools. "
    ...
    "notebook_flow": Scenario(
        name="notebook_flow",
        prompt=(
            "You are running an Nexus notebook test. "
            "You must use tools. "
    ...
    "lsp_flow": Scenario(
        name="lsp_flow",
        prompt=(
            "You are running an Nexus LSP test. "
            "You must use tools. "
    ...
    "cron_flow": Scenario(
        name="cron_flow",
        prompt=(
            "You are running an Nexus cron test. "
            "You must use tools. "
    ...
    "worktree_flow": Scenario(
        name="worktree_flow",
        prompt=(
            "You are running an Nexus worktree test. "
            "You must use tools. "
    ...
    "mcp_auth_flow": Scenario(
        name="mcp_auth_flow",
        prompt=(
            "You are running an Nexus MCP auth reconfiguration test. "
            "You must use tools. "
...
async def _run() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="nexus-e2e-suite-") as temp_dir:
