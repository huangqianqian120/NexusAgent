...
PLUGINS_REPO = Path("/tmp/nexus-test-plugins/plugins")
...
def _run_oh(*args: str, timeout: int = 90, cwd: str | None = None) -> subprocess.CompletedProcess:
    cmd = [sys.executable, "-m", "nexus", *args]
...
async def test_install_real_skills() -> tuple[bool, str]:
    """Copy real skills from anthropics/skills into nexus user skills dir."""
    from nexus.config.paths import get_config_dir
...
async def test_skill_tool_with_real_skill() -> tuple[bool, str]:
    ...
    from nexus.tools.skill_tool import SkillTool, SkillToolInput
...
async def test_skills_in_prompt_with_real() -> tuple[bool, str]:
    ...
    from nexus.prompts.context import build_runtime_system_prompt
...
async def test_install_real_plugins() -> tuple[bool, str]:
    """Copy real plugins into nexus plugin directory."""
    if not PLUGINS_REPO.exists():
        return False, f"Plugins repo not found at {PLUGINS_REPO}"

    dest_base = PROJECT_ROOT / ".nexus" / "plugins"
...
async def test_plugin_hook_structure() -> tuple[bool, str]:
    ...
    dest = PROJECT_ROOT / ".nexus" / "plugins" / "security-guidance"
...
async def test_commit_command_content() -> tuple[bool, str]:
    ...
    dest = PROJECT_ROOT / ".nexus" / "plugins" / "commit-commands"
