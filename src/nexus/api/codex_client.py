...
from nexus.api.errors import AuthenticationFailure, NexusApiError, RateLimitFailure, RequestFailure
from nexus.api.usage import UsageSnapshot
from nexus.engine.messages import ConversationMessage, ImageBlock, TextBlock, ToolResultBlock, ToolUseBlock
...
def _build_codex_headers(token: str, *, session_id: str | None = None) -> dict[str, str]:
    ...
    headers = {
        ...
        "originator": "nexus",
        "User-Agent": f"nexus ({platform.system().lower()} {platform.machine() or 'unknown'})",
...
def _translate_status_error(status_code: int, message: str) -> NexusApiError:
...
class CodexApiClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        body: dict[str, Any] = {
            ...
            "store": False,
            "stream": True,
            "instructions": request.system_prompt or "You are Nexus.",
    ...
    @staticmethod
    def _translate_error(exc: Exception) -> NexusApiError:
