"""GitHub Copilot API client for Nexus.
...
from nexus.api.copilot_auth import (
    copilot_api_base,
    load_copilot_auth,
)
from nexus.api.errors import AuthenticationFailure
...
_VERSION = "0.1.0"  # Nexus version for User-Agent
...
class CopilotClient:
    ...
    github_token:
        ...
        token is loaded from ``~/.nexus/copilot_auth.json``.
    enterprise_url:
        ...
        *,
        ...
        if not token:
            raise AuthenticationFailure(
                "No GitHub Copilot token found. Run 'nx auth copilot-login' first."
            )
