...
from nexus.auth.external import describe_external_binding
from nexus.auth.storage import load_external_binding
from nexus.api.registry import detect_provider_from_registry
...
def auth_status(settings: Settings) -> str:
    ...
    if settings.api_format == "copilot":
        from nexus.api.copilot_auth import load_copilot_auth

        auth_info = load_copilot_auth()
        if not auth_info:
            return "missing (run 'nx auth copilot-login')"
    ...
    try:
        ...
        if settings.provider == "openai_codex":
            return "missing (run 'nx auth codex-login')"
        if settings.provider == "anthropic_claude":
            ...
            return "missing (run 'nx auth claude-login')"
        return "missing"
