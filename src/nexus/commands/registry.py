...
from nexus.bridge.types import WorkSecret
...
from nexus.config.settings import Settings, display_model_setting, load_settings, save_settings
...
from nexus.plugins.installer import install_plugin_from_path, uninstall_plugin
...
if TYPE_CHECKING:
    from nexus.state import AppStateStore
...
def create_default_command_registry(
    ...
    async def _version_handler(_: str, context: CommandContext) -> CommandResult:
        del context
        try:
            version = importlib.metadata.version("nexus")
        except importlib.metadata.PackageNotFoundError:
            version = "0.1.5"
        return CommandResult(message=f"Nexus {version}")
    ...
    async def _init_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        for relative, content in (
            (
                ...
                "# Project Nexus Config\n\nThis directory stores project-specific Nexus state.\n",
            ),
            (
    ...
    async def _login_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(message="Stored API key in ~/.nexus/settings.json")
    ...
    async def _theme_handler(args: str, context: CommandContext) -> CommandResult:
        from nexus.themes import list_themes, load_theme
    ...
    async def _keybindings_handler(_: str, context: CommandContext) -> CommandResult:
        from nexus.keybindings import get_keybindings_path, load_keybindings
    ...
    async def _voice_handler(args: str, context: CommandContext) -> CommandResult:
        from nexus.voice import extract_keyterms, inspect_voice_capabilities
    ...
    async def _privacy_settings_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        lines = [
            ...
            "- storage: local files under ~/.nexus and project .nexus",
        ]
        return CommandResult(message="\n".join(lines))
    ...
    registry.register(SlashCommand("exit", "Exit Nexus", _exit_handler))
