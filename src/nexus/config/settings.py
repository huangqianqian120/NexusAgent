"""Settings model and loading logic for Nexus.
...
3. Config file (~/.nexus/settings.json)
4. Defaults
"""
...
from nexus.hooks.schemas import HookDefinition
from nexus.mcp.types import McpServerConfig
...
class Settings(BaseModel):
    ...
    def resolve_api_key(self) -> str:
        ...
        ``nx auth copilot-login`` and this method is not called.
    ...
    def resolve_auth(self) -> ResolvedAuth:
        ...
        if auth_source in {"codex_subscription", "claude_subscription"}:
            from nexus.auth.external import (
                is_third_party_anthropic_endpoint,
                load_external_credential,
            )
            from nexus.auth.storage import load_external_binding
            ...
            if binding is None:
                raise ValueError(
                    f"No external auth binding found for {auth_source}. Run 'nx auth "
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    if config_path is None:
        from nexus.config.paths import get_config_file_path
