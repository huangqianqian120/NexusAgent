...
from nexus.coordinator.agent_definitions import (
    AGENT_COLORS,
    EFFORT_LEVELS,
    ISOLATION_MODES,
    MEMORY_SCOPES,
...
from nexus.plugins.schemas import PluginManifest
from nexus.plugins.types import LoadedPlugin, PluginCommandDefinition
from nexus.skills.loader import _parse_skill_markdown
from nexus.skills.types import SkillDefinition
...
def get_project_plugins_dir(cwd: str | Path) -> Path:
    ...
    path = Path(cwd).resolve() / ".nexus" / "plugins"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _load_plugin_hooks(path: Path) -> dict[str, list]:
    ...
    if not path.exists():
        return {}
    from nexus.hooks.schemas import (
        AgentHookDefinition,
...
def _load_plugin_mcp(path: Path) -> dict[str, object]:
    ...
    if not path.exists():
        return {}
    from nexus.mcp.types import McpJsonConfig
