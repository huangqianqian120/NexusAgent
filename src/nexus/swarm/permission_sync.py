"""Permission sync protocol for leader-worker coordination in Nexus swarms.
...
Paths:
    ~/.nexus/teams/<teamName>/permissions/pending/<id>.json
    ~/.nexus/teams/<teamName>/permissions/resolved/<id>.json
"""
...
from nexus.swarm.lockfile import exclusive_file_lock
from nexus.swarm.mailbox import (
    MailboxMessage,
    TeammateMailbox,
...
if TYPE_CHECKING:
    from nexus.permissions.checker import PermissionChecker
...
def get_permission_dir(team_name: str) -> Path:
    """Return ~/.nexus/teams/{teamName}/permissions/"""
...
async def get_leader_name(team_name: str | None = None) -> str | None:
    ...
    """
    from nexus.swarm.team_lifecycle import read_team_file_async
...
async def handle_permission_request(
    ...
    Args:
        ...
        checker: An already-configured :class:`~nexus.permissions.checker.PermissionChecker`.
