...
_TEAMMATE_ENV_VARS = [
    ...
    "REQUESTS_CA_BUNDLE",
    "CURL_CA_BUNDLE",
    # --- Nexus-native provider settings --------------------------------
...
def get_teammate_command() -> str:
    ...
    2. The ``nexus`` entry-point on PATH (installed package mode).
    3. The current Python interpreter running the ``nexus`` module
    ...
    entry_point = shutil.which("nexus")
    if entry_point:
        return entry_point
