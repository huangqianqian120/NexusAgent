"""Persistent team lifecycle management for Nexus swarms.

Teams are stored as JSON files on disk:
    ~/.nexus/teams/<name>/team.json
...
from nexus.swarm.mailbox import get_team_dir
from nexus.swarm.types import BackendType
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    """
    from nexus.swarm.registry import get_backend_registry
    from nexus.swarm.spawn_utils import is_inside_tmux
    from nexus.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.nexus/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.nexus/teams/``, sorted by name."""
        base = Path.home() / ".nexus" / "teams"
        if not base.exists():
            return []
