...
from nexus.api.client import AnthropicApiClient, SupportsStreamingMessages
...
from nexus.api.provider import auth_status, detect_provider
from nexus.bridge import get_bridge_manager
from nexus.commands import CommandContext, CommandResult, create_default_command_registry
from nexus.config import get_config_file_path, load_settings
from nexus.config.settings import display_model_setting
from nexus.engine import QueryEngine
from nexus.engine.messages import ConversationMessage, ToolResultBlock, ToolUseBlock
from nexus.engine.query import MaxTurnsExceeded
from nexus.engine.stream_events import StreamEvent
from nexus.hooks import HookEvent, HookExecutionContext, HookExecutor, load_hook_registry
from nexus.hooks.hot_reload import HookReloader
from nexus.mcp.client import McpClientManager
from nexus.mcp.config import load_mcp_server_configs
from nexus.permissions import PermissionChecker
from nexus.plugins import load_plugins
from nexus.prompts import build_runtime_system_prompt
from nexus.state import AppState, AppStateStore
from nexus.services.session_backend import DEFAULT_SESSION_BACKEND, SessionBackend
from nexus.tools import ToolRegistry, create_default_tool_registry
from nexus.keybindings import load_keybindings
...
class RuntimeBundle:
    ...
    def current_settings(self):
        ...
        We persist most settings to disk (``~/.nexus/settings.json``), but
...
def _resolve_api_client_from_settings(settings) -> SupportsStreamingMessages:
    ...
    def _safe_resolve_auth():
        try:
            ...
            print(
                ...
                "  Run `nx auth login` to set up authentication, or set the\n"
    ...
    if settings.api_format == "copilot":
        from nexus.api.copilot_client import COPILOT_DEFAULT_MODEL
...
async def build_runtime(
    *,
    ...
    """Build the shared runtime for an Nexus session."""
...
async def _render_command_result(
    ...
    if result.replay_messages and render_event is not None:
        ...
        from nexus.engine.stream_events import AssistantTextDelta, AssistantTurnComplete
